...
contains
  ...
  subroutine initialize(previous, current, nsteps, iter0)
    use utilities
    use io
  ...
  subroutine generate_field(field0)
    ...
    field0%data(:,0) = 20.0_dp           ! Left boundary
    field0%data(0,:) = 85.0_dp           ! Upper boundary
    field0%data(:,field0%ny+1) = 70.0_dp ! Right boundary
    field0%data(field0%nx+1,:) = 5.0_dp  ! Lower boundary
