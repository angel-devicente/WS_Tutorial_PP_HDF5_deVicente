  ...
  subroutine write_field(curr, iter, parallel)
    ...
    if (parallel%rank == 0) then
       ...
       do p = 1, parallel%size - 1
          ...
          ! Getting coords in cartesian topology manually
          if (mod(p, 2) == 1) then
             coords(2) = 1        ! right boundary
          else
             coords(2) = 0        ! left boundary
          end if

          if ((p / 2) == 1) then
             coords(1) = 1        ! bottom boundary
          else
             coords(1) = 0        ! top boundary
          end if

          ix = coords(1) * curr%nx + 1
          jy = coords(2) * curr%ny + 1
          call mpi_recv(full_data(ix:ix+curr%nx-1, jy:jy+curr%ny-1), curr%nx*curr%ny, MPI_DOUBLE_PRECISION,&
               p, 22, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
       end do
  ...
  subroutine read_field(field0, filename, parallel)
    ...
    if (parallel%rank == 0) then
       ...
       do p=1, parallel%size - 1
          ...
          call mpi_send(full_data(ix:ix+field0%nx-1, jy:jy+field0%ny-1), field0%nx*field0%ny, &
               MPI_DOUBLE_PRECISION,p,44,MPI_COMM_WORLD,ierr)
       end do
    else
       ! Receive data
       call mpi_recv(field0%data(1:field0%nx,1:field0%ny), field0%nx*field0%ny, MPI_DOUBLE_PRECISION, 0, 44, &
